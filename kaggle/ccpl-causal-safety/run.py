...
subprocess.run([sys.executable, "ccpl_experiments.py", "--exp", "E3",
                *common], cwd=repo, check=True)

subprocess.run([sys.executable, "-m", "pip", "install", "safety-gymnasium==1.0.0",
                "mujoco>=3.0", "gymnasium>=0.29"], check=True)
for experiment in ("E8", "E9"):
    subprocess.run([sys.executable, "ccpl_experiments.py", "--exp", experiment,
                    *common], cwd=repo, check=True)

archive = Path("/kaggle/working/ccpl_causal_safety_results.tar.gz")
